...
from core.canvas_fitter import process_canvas_folder

class CanvasTab(tk.Frame):
    """
    Aba que implementa a interface e controle para a funcionalidade de Canvas Fit.
    Permite emoldurar imagens em tamanhos fixos com redimensionamento proporcional interno.
    """
    BG_COLORS = {
    ...
    def __init__(self, parent, log_panel, progress_bar):
        ...
        # Frame de configurações de dimensão
        ...
        # Aviso sobre fundos cromakey na UI
        tk.Label(
            self.container, 
    ...
    def _on_input_changed(self, *args) -> None:
        """Sugerir pasta de saída padrão quando a entrada for alterada."""
    ...
    def _validate_int_or_auto(value: str) -> bool:
        """Permite apenas números ou a palavra AUTO."""
        return value == "" or value == "AUTO" or (value.isdigit() and int(value) >= 0)

    def _toggle_inner_w(self) -> None:
        """Alterna a entrada de largura entre editável e AUTO."""
    ...
    def _toggle_inner_h(self) -> None:
        """Alterna a entrada de altura entre editável e AUTO."""
    ...
    def _start_processing(self) -> None:
        """Inicia o processamento assincrono do Canvas Fit."""
        ...
        # Leitura e conversão das dimensões do canvas
        try:
        ...
        # Leitura dos limites internos (tratando AUTO como None)
        inner_w = None
        if not self.inner_w_auto.get():
            try:
                ...
                messagebox.showwarning("Atenção", "Informe um valor numérico válido maior que 0 para a largura interna.")
                return
        ...
        if not self.inner_h_auto.get():
            try:
                ...
                messagebox.showwarning("Atenção", "Informe um valor numérico válido maior que 0 para a altura interna.")
                return

        # Pega a tupla da cor selecionada
        color_name = self.bg_color_var.get()
        bg_color = self.BG_COLORS.get(color_name, (0, 0, 0, 0))

        suffix_pattern = self.suffix_var.get().strip()

        # Desativa controles e limpa painéis
